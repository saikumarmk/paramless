...
"""
_within_bounds(vector, lower_bound, upper_bound)

Checks if all elements of an array are contained within [lower_bound, upper_bound]


"""
function _within_bounds(vector::Array, lower_bound, upper_bound)
...
function point_mutation(vector::Array; kwargs...)::Array
    ...
    while !is_inside
        ...
        is_inside = _within_bounds(mutant, lower_bound, upper_bound) # Check if everything is contained
        attempt += 1
...
# IMPLEMENT GAUSSIAN HGELPER 

function _gaussian_mutation_helper(x, mutation_epsilon::Float64, loc::Float64, width::Float64)
...
function gaussian_mutation_distribution(vector::Array, mutation_epsilon::Float64, domain, width)::Array
    #= moves up a random point, and down a random point at the same time
